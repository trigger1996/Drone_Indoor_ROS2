...
class ROSReceiver:
    def __init__(self, app):
        ...
        for i in range(1, self.uavNum + 1):
            ...
            # 新增姿态订阅
            cmd_att_topic_name = "/cmd_attitude_%d" % (i,)
            cmd_att_sub = rospy.Subscriber(cmd_att_topic_name, AttitudeSetpoint2,
                                           self.cmd_att_callback, callback_args=i)
            self.cmd_att_subscribers.append(cmd_att_sub)
            print("ROS Receiver initialized, listening to %s" % cmd_att_topic_name)
    ...
    def cmd_att_callback(self, msg, drone_index):
        drone = self.drones[drone_index-1]
        drone.cmdAtt[0] = msg.roll_body
        drone.cmdAtt[1] = msg.pitch_body
        drone.cmdAtt[2] = msg.yaw_body
        drone.cmdAtt[3] = msg.roll_rate
        drone.cmdAtt[4] = msg.pitch_rate
        drone.cmdAtt[5] = msg.yaw_rate
        drone.cmdAtt[6] = msg.thrust_body
        drone.cmd_att_update = True
        drone.matlabControlInited = True

        print("Received /cmd_attitude_%d: roll:%f pitch:%f yaw:%f rates:(%f,%f,%f) thrust:%f" %
              (drone_index, msg.roll_body, msg.pitch_body, msg.yaw_body,
               msg.roll_rate, msg.pitch_rate, msg.yaw_rate, msg.thrust_body))
...
class Drone():
    def __init__(self, addr,nId):
        ...
        self.cmdlast = 0
        # Added
        self.cmdAtt = [0]*7   # roll, pitch, yaw, roll_rate, pitch_rate, yaw_rate, thrust
        self.cmd_att_update = False
        #
    ...
    def update(self):
        if self.cmd_vel_update:
        ...
        elif self.cmd_att_update:

            self.cmd_att_update = False
            roll, pitch, yaw = self.cmdAtt[0:3]
            roll_rate, pitch_rate, yaw_rate = self.cmdAtt[3:6]
            thrust = self.cmdAtt[6]
            ...
            print("send attitude to uav %d==roll:%f pitch:%f yaw:%f | rates:(%f,%f,%f) | thrust:%f"
                % (self.nId, roll, pitch, yaw, roll_rate, pitch_rate, yaw_rate, thrust))
        else:
